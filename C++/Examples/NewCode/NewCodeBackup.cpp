...
#define MAX_DEFLECTION 200 // 0.2 // this is used for rigging
#define LINE_NEUTRAL 0 // 1.500 // center point for the actuator, range is -1000-1000
...
//					winch_right_cmd = LINE_NEUTRAL + cmd_adapt;
			...
			//cout << "serial_port value: " << serial_port << endl;
			//cout << endl;
			winch_left_cmd = 500;

			std::string serialmotor1;
			//serialmotor1 = "!G 1 -1000\r";
			serialmotor1 = "!G 1 ";
			//serialmotor1 += "1000";
			serialmotor1 += std::to_string(winch_left_cmd);
			cout << std::to_string(winch_left_cmd) << endl;
			serialmotor1 += "\r";
			//cout << winch_left_cmd << endl;
			const char * charmotor1 = serialmotor1.c_str();
			serialPuts(serial_port,charmotor1);
			//serialPuts(serial_port,"!G 1 1000\r");
			cout << "command 1 sent" << endl;
			serial_output = serialGetchar(serial_port);
			serial_output = serialGetchar(serial_port);

			serialPuts(serial_port,"?M 1\r");
			cout << "commanded position 1" << endl;
			serial_output = serialGetchar(serial_port);
			serial_output = serialGetchar(serial_port);
			...
			enc1 += serial_output;}
			cout << stoi(enc1) << endl;
			cout << "serial chars available: " << serialDataAvail(serial_port) << endl;

			std::string serialmotor2;
			//serialmotor2 = "!G 2 -1000\r";
			serialmotor2 = "!G 2 ";
			serialmotor2 += "1000";
			serialmotor2 += "\r";
			//cout << winch_right_cmd << endl;
			const char * charmotor2 = serialmotor2.c_str();
			serialPuts(serial_port,charmotor2);
			//serialPuts(serial_port,"!G 2 1000\r");
			cout << "command 2 sent" << endl;
			serial_output = serialGetchar(serial_port);
			serial_output = serialGetchar(serial_port);

			serialPuts(serial_port,"?M 2\r");
			cout << "commanded position 2" << endl;
			serial_output = serialGetchar(serial_port);
			serial_output = serialGetchar(serial_port);
			...
			//cout << serialGetchar(serial_port) << endl;
			//cout << serialGetchar(serial_port) << endl;

			//writeG1 = time.time() -writeG1
			//readG1 = time.time()
			//ser.read_until(b'\r')
