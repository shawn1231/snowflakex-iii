...
//------------------------------------------------------------------------------------------------------------------PID Variables
char control_type = 's'; // valid options:  p = PID , n = NDI , g = glide, no spin, 'm' = multisine , 's' = step input , c = rc control
...
//const float output_range[6][2] ={{-kd_start*LIVE_GAIN_SCALE,kd_start*LIVE_GAIN_SCALE},{-ki_start*LIVE_GAIN_SCALE,ki_start*LIVE_GAIN_SCALE},{-kp_start*LIVE_GAIN_SCALE,kp_start*LIVE_GAIN_SCALE},{-.25,.25},{-100,100},{0,2}};
//const float output_range[6][2] = {{1/LIVE_GAIN_SCALE,LIVE_GAIN_SCALE},{1/LIVE_GAIN_SCALE,LIVE_GAIN_SCALE},{1/LIVE_GAIN_SCALE,LIVE_GAIN_SCALE},{-.25,.25},{-100,100},{0,2}};

// for PID tuning
const float output_range[6][2] = {{-.05,.30},{2,-2},{-.185,.500},{-.0024,.04},{-.1,.1},{-.1,.1}};
...
{
	//----------------------------------------------------------------------------------------------------------------File Title Prefix
	// reduce by order of magnitude
	//kp_start = kp_start/10;
	//ki_start = ki_start/10;
	//kd_start = kd_start/10;
	...
	const float frequency [NUM_LOOPS] = {300,1,1,100,50,20,10,1,1}; //Hz
...
//			ahrs_lsm_madgwick.updateMadgwickIMU(a_lsm_ahrs[0],a_lsm_ahrs[1],a_lsm_ahrs[2],g_lsm_ahrs[0],g_lsm_ahrs[1],g_lsm_ahrs[2],dt);
		...
		if( (time_now-timer[3]) > duration[3])
		{
			...
			if(yaw_mpu_madgwick > 140 && yaw_prev < -140){
				num_wraps--;
			}
			if(yaw_mpu_madgwick < -140 && yaw_prev > 140){
				num_wraps++;
			...
			//yaw_error             = yaw_desired - yaw_mpu_madgwick;
...
//			yaw_error             = yaw_desired - yaw_lsm_integrated + 360*num_wraps;
...
//			fout << g_lsm[2] << "," << m_mpu[0] << "," << m_mpu[1] << "," << m_mpu[2] << "," << m_lsm[0] << "," << m_lsm[1] << "," << m_lsm[2] << endl;
			...
			if(!dbmsg_global && !dbmsg_local)
			{
				...
				cout << "Euler Angles (Madgwick): " << dt << endl;
				...
				cout << "\tSystem ID, A: " << A << " B: " << B << " C: " << C << endl;
